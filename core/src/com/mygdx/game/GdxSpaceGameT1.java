...
public class GdxSpaceGameT1 implements ApplicationListener {
    ...
    private Sound shootSound;//sound for when shooting
    private Sound hitSound;//sound for when hit
    private Sound gameOverSound;//sound for going to game over screen
...
//        enemies.add(new Enemy(sprt, 100, 0,  (float)(screenSizeX*.037), (float)(screenSizeY*.055)));
        ...
        //For music playing in the background when game opens
        music = Gdx.audio.newMusic(Gdx.files.internal("backgroundmusic.mp3"));
        music.setLooping(true);
        music.setVolume(0.5f);


        //For shoot sound and impact sound
    ...
    //To verify coordinates with mouse
        ...
        else if (rightPressed && unPauseFrmCounter>60 && mouseX2 > pause.sprt.getX()
        ...
        if (leftPressed && (bulletShotCounter % 15 == 0) && mouseX > shoot.sprt.getX()
                ...
                && (mouseY - (float)((screenSizeY*1.528))) <= (shoot.posY + shoot.sprt.getHeight() ) && (mouseY - (float)((screenSizeY*1.528))) >= (shoot.posY)
        ){
        ...
        //If mouse is pressed on shoot button, shoot
        if (rightPressed && (bulletShotCounter % 15 == 0) && mouseX2 > shoot.sprt.getX()
                && mouseX2 < (shoot.sprt.getX()+ shoot.sprt.getWidth())
                && (mouseY2 - (float)((screenSizeY*1.528))) <= (shoot.posY + shoot.sprt.getHeight() ) && (mouseY2 - (float)((screenSizeY*1.528))) >= (shoot.posY)
        ){
            ship.shoot(rightPressed);
            shootSound.play(0.15f);
        }
        ...
        for(int i = 0; i < Bullet.enemyBullets.size(); i++){
            ...
            if (bulletPosX < ship.posX + ship.sprt.getWidth()/2 && bulletPosX > ship.posX   && bulletPosY < ship.posY + ship.sprt.getHeight()/2 && bulletPosY > ship.posY){
    ...
    public void drawGameOver() {
        ...
        batch.begin();
        font.draw(batch,
                "GAME OVER!\nYOU ELIMINATED: "+killcount+" ENEMIES.\nTouch the screen to keep playing...",
                camera.position.x,
                camera.position.y);
